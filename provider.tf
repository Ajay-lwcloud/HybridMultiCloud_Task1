...
resource "aws_instance" "web" {
  ...
  connection {
    type        = "ssh"
    user        = "ec2-user"
    private_key = file("Path_Of_Your_key_File")
...
provisioner "remote-exec" {
        inline  = [
            ...
            "sudo su << EOF",
            "echo \"<center><img src='http://${self.domain_name}/${aws_s3_bucket_object.object.key}' height='150px' width='500px'></center>\" >> /var/www/html/index.html",
            "EOF"
        ]
    }
...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone Your_GitHub_Address_Here /var/www/html",
    ]
  }
}
...
depends_on = [
	...
	provisioner "local-exec" {
	    command = "start (Your_Browser_Here)  ${aws_instance.web.public_ip}"
  	}
}
